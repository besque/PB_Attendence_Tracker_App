          ...
          IconButton(
            ...
            onPressed: () async {
              try{
                ...
                final csvData = await _eventService.exportEventToCsv(widget.eventId);

                // temporary directory to save the file
                final directory = await getTemporaryDirectory();
                final path = '${directory.path}/${widget.eventName}_attendance.csv';
            ...
            child: Wrap(
              ...
              children: [
                ChoiceChip(
                  ...
                  onSelected: (selected) => setState(() => _sortOption = AttendeeSortOption.checkInTime),
                ),
                ChoiceChip(
                  label: const Text('Sort by Name'),
                  selected: _sortOption == AttendeeSortOption.name,
                  onSelected: (selected) => setState(() => _sortOption = AttendeeSortOption.name),
                ),
                ChoiceChip(
                  label: const Text('Sort by Team'),
                  selected: _sortOption == AttendeeSortOption.teamId,
                  onSelected: (selected) => setState(() => _sortOption = AttendeeSortOption.teamId),
                ),
                ChoiceChip(
                  label: const Text('Affiliation Type'),
                  selected: _sortOption == AttendeeSortOption.affiliationType,
                  onSelected: (selected) => setState(() => _sortOption = AttendeeSortOption.affiliationType),
                ),
                ChoiceChip(
                  label: const Text('Experience'),
                  selected: _sortOption == AttendeeSortOption.experienceLevel,
                  onSelected: (selected) => setState(() => _sortOption = AttendeeSortOption.experienceLevel),
                ),
                ChoiceChip(
                  label: const Text('Age'),
                  selected: _sortOption == AttendeeSortOption.age,
                  onSelected: (selected) => setState(() => _sortOption = AttendeeSortOption.age),
                ),    
              ],
            ),
          ),
          Expanded(
            child: StreamBuilder<QuerySnapshot>(
              ...
              builder: (context, snapshot) {
                ...
                if (!snapshot.hasData || snapshot.data!.docs.isEmpty) return const Center(child: Text('No attendees found.'));
                ...
                if (query.isNotEmpty) {
                  participants = participants.where((doc) {
                    ...
                    final nameLower = (data['participant_name'] as String? ?? '').toLowerCase();
                    final emailLower = (data['participant_email'] as String? ?? '').toLowerCase();
                ...
                participants.sort((a, b) {
                  ...
                  switch (_sortOption) {
                    case AttendeeSortOption.checkInTime:
                      final timeA = dataA['check_in_time'] as Timestamp? ?? Timestamp(0, 0);
                      final timeB = dataB['check_in_time'] as Timestamp? ?? Timestamp(0, 0);
                      return timeB.compareTo(timeA);
                    case AttendeeSortOption.name:
                       return (dataA['participant_name'] as String? ?? '').compareTo(dataB['participant_name'] as String? ?? '');
                    case AttendeeSortOption.teamId:
                       return (dataA['team_id'] as String? ?? '').compareTo(dataB['team_id'] as String? ?? '');
                    case AttendeeSortOption.affiliationType:
                       return (dataA['affiliationType'] as String? ?? '').compareTo(dataB['affiliationType'] as String? ?? '');
                    case AttendeeSortOption.experienceLevel:
                       return (dataA['experienceLevel'] as String? ?? '').compareTo(dataB['experienceLevel'] as String? ?? '');
                    case AttendeeSortOption.age:
                       return (dataA['age'] as int? ?? 999).compareTo(dataB['age'] as int? ?? 999);
                  }
                });
                ...
                return ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return ListTile(
                      ...
                      title: Text(participantData['participant_name'] ?? 'Unknown'),
                      subtitle: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          Text(participantData['participant_email'] ?? 'No email'),
                          Text(
                      ...
                      trailing: Text(
                        ...
                        formatCheckInTime((participantData['check_in_time'] as Timestamp?)?.toDate().toIso8601String()),
